...
public class EvaluationController : MonoBehaviour
{
    ...
    private IEnumerator Evaluate()
    {
        if (null != EvaluationResponses && EvaluationResponses.Any())
        {
            ...
            ResponseFeedbackIncorrect.SetActive(false);
            ResponseFeedbackCorrect.SetActive(false);
            Dialog myDialog = Dialog.Open(_mediumDialogPrefab, DialogButtonType.OK, "<align=\"center\">Resultados", EvaluationResponses.Aggregate("", (current, next) => $"{current}<align=\"center\">- {next.Expression.Word} {(next.IsCorrect ? "<color=\"green\">Correcto</color>" : "<color=\"red\">Incorrecto</color>")}\n"), true);
            if (myDialog != null)
            {

...
internal static class SceneUnderstandingHelper 
{
        ...
        float searchRadius = 3.0f)
    {
        ...
        var sceneBuffer = Resources.Load<TextAsset>(Path.Combine("TestAssets", "SU_Apartment")).bytes;
        Scene scene = Scene.Deserialize(sceneBuffer);
        if (scene != null)
        {
            ...
            var largestPlatform = scene.LargestSceneObjectOfType(SceneObjectKind.Wall);
